...
public class KMEcdsa256NoDigestSignature extends Signature implements Shareable {
  ...
  public KMEcdsa256NoDigestSignature(byte alg) {
    algorithm = alg;
    inst = Signature.getInstance(Signature.ALG_ECDSA_SHA_256, true);
  }

  public KMEcdsa256NoDigestSignature(byte alg, Signature inst) {
    algorithm = alg;
    this.inst = inst;
  }
  ...
  public short sign(byte[] bytes, short i, short i1, byte[] bytes1, short i2)
    ...
    try {
      ...
      //byte[] hash = new byte[(byte)32];
      //byte[] sign = new byte[(byte)100];
      ...
      //inst.signPreComputedHash(hash, (byte)0, (byte)32, sign, (byte)0);
      //return (byte)72;
      return inst.signPreComputedHash(bytes, i, i1, bytes1, i2);
    } finally {
