...
public class KMAndroidSEProvider implements KMSEProvider {
  ...
  public void createAsymmetricKey(byte alg, byte[] privKeyBuf,
    ...
    switch (alg) {
      ...
      case KMType.EC:
        ...
        if(ecPrivKey != null && ecPrivKey.isInitialized()) {
        	lengths[0] = ecPrivKey.getS(privKeyBuf, privKeyStart);
        }
        if(ecPubKey != null && ecPubKey.isInitialized())
  ...
  public short aesGCMEncrypt(AESKey key,
    ...
    try{ aesGcmCipher.init(key, Cipher.MODE_ENCRYPT, nonce, nonceStart, nonceLen); }catch(Exception e) {ISOException.throwIt((short)0x4200);}
    try{ aesGcmCipher.updateAAD(authData, authDataStart, authDataLen); }catch(Exception e) {ISOException.throwIt((short)0x4300);}
    short ciphLen = 0;
    try{ ciphLen = aesGcmCipher.doFinal(secret, secretStart, secretLen,
        encSecret, encSecretStart); }catch(Exception e) {ISOException.throwIt((short)0x4400);}
    try{ aesGcmCipher.retrieveTag(authTag, authTagStart, authTagLen); }catch(Exception e) {ISOException.throwIt((short)0x4500);}
    return ciphLen;
  }
  ...
  public short aesGCMEncrypt(byte[] aesKey, short aesKeyStart, short aesKeyLen,
    ...
    AESKey key = null;
	try{ key = createAESKey(aesKey, aesKeyStart, aesKeyLen); }catch(Exception e) {ISOException.throwIt((short)0x4100);}
  ...
  public boolean aesGCMDecrypt(byte[] aesKey, short aesKeyStart,
    ...
    AESKey key = null;
    try { key = createAESKey(aesKey, aesKeyStart, aesKeyLen); }catch(Exception e) {ISOException.throwIt((short)0x5100);}
    try{ aesGcmCipher.init(key, Cipher.MODE_DECRYPT, nonce, nonceStart, nonceLen); }catch(Exception e) {ISOException.throwIt((short)0x5200);}
    try{ aesGcmCipher.updateAAD(authData, authDataStart, authDataLen); }catch(Exception e) {ISOException.throwIt((short)0x5300);}
    // encrypt the secret
    try{ aesGcmCipher.doFinal(encSecret, encSecretStart, encSecretLen, secret,
        secretStart); }catch(Exception e) {ISOException.throwIt((short)0x5400);}
    try{ verification = aesGcmCipher.verifyTag(authTag, authTagStart, (short) authTagLen,
        (short) AES_GCM_TAG_LENGTH); }catch(Exception e) {ISOException.throwIt((short)0x5500);}
    return verification;
  }
  ...
  public Signature createEcSigner(short digest, byte[] secret,
    ...
    //ecSigner = Signature.getInstance(Signature.ALG_ECDSA_SHA_256, true);
    //ecSigner = new KMEcdsa256NoDigestSignature(alg, ecSigner);
