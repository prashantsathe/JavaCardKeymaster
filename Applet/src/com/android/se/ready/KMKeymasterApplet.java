...
public class KMKeymasterApplet extends Applet implements AppletEvent, ExtendedLength {
  ...
  public static void install(byte[] bArray, short bOffset, byte bLength) {
    new KMKeymasterApplet(null).register(bArray, (short) (bOffset + 1), bArray[bOffset]);
  }
  ...
  private void processDeviceLockedCmd(APDU apdu) {
    ...
    KMArray.cast(tmpVariables[0]).add((short) 1, KMVerificationToken.exp());
  ...
  private void processProvisionAttestationCertParams(APDU apdu) {
    ...
    KMArray.cast(argsProto).add((short) 0, KMByteBlob.exp()); // Cert - DER encoded issuer
    KMArray.cast(argsProto).add((short) 1, KMByteBlob.exp()); // Cert - Expiry Time
    // Decode the argument.
    short args = decoder.decode(argsProto, (byte[]) bufferRef[0], bufferProp[BUF_START_OFFSET], bufferProp[BUF_LEN_OFFSET]);
    //reclaim memory
    repository.reclaimMemory(bufferProp[BUF_LEN_OFFSET]);
  ...
  public short createAttestationForEcPublicKey(boolean isTestCredential,
		  ...
		  short attChallangeOffset, short attChallangeLen,
	  ...
	  return Util.arrayCopyNonAtomic((byte[])bufferRef[0], cert.getCertStart(), scratchPad, scratchPadOffset, cert.getCertLength());
  }
  ...
  private void processFinishOperationCmd(APDU apdu) {
    ...
    // Arguments
    KMArray.cast(tmpVariables[1]).add((short) 0, KMInteger.exp());
    KMArray.cast(tmpVariables[1]).add((short) 1, KMKeyParameters.exp());
    KMArray.cast(tmpVariables[1]).add((short) 2, KMByteBlob.exp());
    KMArray.cast(tmpVariables[1]).add((short) 3, KMByteBlob.exp());
    KMArray.cast(tmpVariables[1]).add((short) 4, KMHardwareAuthToken.exp());
    KMArray.cast(tmpVariables[1]).add((short) 5, KMVerificationToken.exp());
    // Decode the arguments
    tmpVariables[2] = decoder.decode(tmpVariables[1], (byte[]) bufferRef[0], bufferProp[BUF_START_OFFSET], bufferProp[BUF_LEN_OFFSET]);
    //reclaim memory
    repository.reclaimMemory(bufferProp[BUF_LEN_OFFSET]);
  ...
  private void processUpdateOperationCmd(APDU apdu) {
    ...
    KMArray.cast(tmpVariables[1]).add((short) 1, KMKeyParameters.exp());
    KMArray.cast(tmpVariables[1]).add((short) 2, KMByteBlob.exp());
    KMArray.cast(tmpVariables[1]).add((short) 3, KMHardwareAuthToken.exp());
    KMArray.cast(tmpVariables[1]).add((short) 4, KMVerificationToken.exp());
    // Decode the arguments
    tmpVariables[2] = decoder.decode(tmpVariables[1], (byte[]) bufferRef[0], bufferProp[BUF_START_OFFSET], bufferProp[BUF_LEN_OFFSET]);
    //reclaim memory
    repository.reclaimMemory(bufferProp[BUF_LEN_OFFSET]);
  ...
  private void processBeginOperationCmd(APDU apdu) {
    ...
    KMArray.cast(tmpVariables[1]).add((short) 2, KMKeyParameters.exp());
    KMArray.cast(tmpVariables[1]).add((short) 3, KMHardwareAuthToken.exp());
    // Decode the arguments
  ...
  private void processImportKeyCmd(APDU apdu) {
    ...
    // Arguments
    KMArray.cast(tmpVariables[1]).add((short) 0, KMKeyParameters.exp());
  ...
  private static void makeKeyCharacteristics(byte[] scratchPad) {
    ...
    KMKeyCharacteristics.cast(data[KEY_CHARACTERISTICS]).setEnforced(data[HW_PARAMETERS], KMKeyCharacteristics.HARDWARE_ENFORCED);
    KMKeyCharacteristics.cast(data[KEY_CHARACTERISTICS]).setEnforced(data[SW_PARAMETERS], KMKeyCharacteristics.SOFTWARE_ENFORCED);
  }
  ...
  private static void parseEncryptedKeyBlob(byte[] scratchPad) {
    ...
    try {
      ...
      data[HW_PARAMETERS] = KMKeyCharacteristics
          .cast(data[KEY_CHARACTERISTICS]).getEnforced(KMKeyCharacteristics.HARDWARE_ENFORCED);
      data[SW_PARAMETERS] = KMKeyCharacteristics
          .cast(data[KEY_CHARACTERISTICS]).getEnforced(KMKeyCharacteristics.SOFTWARE_ENFORCED);
