...
public class KMTag extends KMType {
  ...
  public static boolean isValidPublicExponent(short params) {
    short pubExp = KMKeyParameters.findTag(ULONG_TAG, RSA_PUBLIC_EXPONENT, params);
    if(pubExp == INVALID_VALUE){
      return false;
    }
  ...
  public static boolean isValidKeySize(short params){
    short keysize = KMKeyParameters.findTag(UINT_TAG, KEYSIZE, params);
    if(keysize == INVALID_VALUE){
      return false;
    }
    short alg = KMEnumTag.getValue(ALGORITHM, params);
